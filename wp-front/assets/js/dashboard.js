/**
 * Finlyzer — Dashboard Client Controller (v1.13.0)
 *
...
(function () {
	...
	// dynamically inject WordPress REST nonce into all outgoing htmx requests
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		// -------------------------------------------------------------
		// CONNECTION LIFECYCLE & RETRY STATE MACHINE
		// -------------------------------------------------------------
		var MAX_ATTEMPTS = 3;
		...
		function setConnectingState(attempt) {
			...
			if (connText) {
				if (attempt > 0) {
					...
					connText.textContent = 'Connecting to Finlyzer calculation API...';
				}
			}
		}
		...
		function fetchData(days) {
			...
			if (window.htmx) {
				// dispatch via htmx ajax for reliable dom swap and lifecycle events
				window.htmx.ajax('GET', summaryUrl, { target: summary, swap: 'innerHTML' });
				window.htmx.ajax('GET', insightUrl, { target: insight, swap: 'innerHTML' });
			} else {
				...
				Promise.all([
					...
					fetch(insightUrl, { headers: headers }).then(function (r) {
						if (!r.ok) throw new Error('HTTP ' + r.status);
						return r.text();
					}),
				])
		...
		// intercept htmx lifecycle events for connection monitoring
		...
		// initialize connection state on initial page load
		setConnectingState(0);
	});
})();
